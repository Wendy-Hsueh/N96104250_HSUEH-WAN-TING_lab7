        ...
        while run:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    # if sound button is clicked music valume turn to 0.2
                    if self.sound_btn.clicked(x, y):
                        pygame.mixer.music.set_volume(0.2)
                    # if mute button is clicked music valume turn to 0
            ...
            for btn in self.buttons:    # the button that user click in button list
                if btn.clicked(x, y):   # check button was clicked or not
                    btn.create_frame(x, y)  # create frame size
                    btn.draw_frame(self.menu_win)   # draw the frame
...
class Buttons:
    ...
    def create_frame(self, x: int, y: int):
        ...
        if self.clicked(x, y):
            x, y, w, h = self.rect
            self.frame = pygame.Rect(x - 5, y - 5, w + 10, h + 10)  # frame size
        else:
            self.frame = None

    def draw_frame(self, win):
        if self.frame is not None:
            pygame.draw.rect(win, WHITE, self.frame, 10)    # draw frame
